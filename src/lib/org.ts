...
async function resolveOrgId(): Promise<string | null> {
  ...
  // A função vive em public (via MOVER-PARA-PUBLIC.sql) — chamada direta,
  // sem prefixo de schema, sem header Accept-Profile, sem 406.
  const { data, error } = await supabase.rpc("create_org_for_user", { _name: ORG_NAME });
