...
import { CLIENT_SYSTEM_PROMPT, CLIENT_IMAGE_DIRECTIVE } from "./client-context.ts";
...
export async function openaiChat(
  ...
  const body: Record<string, unknown> = {
    ...
    messages: [
      {
        role: "system",
        // O contexto do cliente (Mamma Jamma) é SEMPRE a base. Se a função
        // passar um system próprio, ele entra como camada adicional acima do
        // contexto do cliente — nunca o substitui. Assim toda geração já sai
        // no tom e nas regras da marca, sem o operador repetir nada.
        content: opts.system
...
export async function openaiImage(
  ...
  // Toda imagem herda a direção visual da marca (Mamma Jamma).
